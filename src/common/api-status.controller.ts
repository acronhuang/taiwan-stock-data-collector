...
export class ApiStatusController {
  ...
  checkApiStatus(
    ...
    const description = this.apiStatusService.getIssueDescription(date, apiName);
  ...
  getDashboard() {
    ...
    return {
      timestamp: new Date().toISOString(),
      overview: {
        totalApis: health.healthy.length + health.issues.length + health.monitoring.length,
  ...
  private generateRecommendations(health: any, currentIssues: any[]): string[] {
    ...
    if (health.monitoring.length > 0) {
      recommendations.push(`持續監控 ${health.monitoring.join(', ')} API的穩定性`);
    }
