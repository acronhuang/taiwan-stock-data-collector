...
async function bootstrap() {
  ...
  Logger.log(`🚀 開始抓取完整10月資料 (${startDate.toISODate()} 到 ${endDate.toISODate()})`);
  ...
  for (let dt = startDate; dt <= endDate; dt = dt.plus({ day: 1 })) {
    ...
    if (processedDays % 5 === 0 || processedDays === totalDays) {
      ...
      Logger.log(`📊 進度: ${progress}% (成功:${successDays}, 失敗:${errorDays})`);
    }
  ...
  Logger.log(`   - 成功率: ${Math.round((successDays/processedDays)*100)}%`);
