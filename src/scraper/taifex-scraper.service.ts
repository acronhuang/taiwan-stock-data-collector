...
export class TaifexScraperService {
   ...
   * 解析 TXO Calls 未平倉資料
   */
  private parseTxoCallsData(txoRows: string[][]) {
   ...
   * 解析 TXO Puts 未平倉資料
   */
  private parseTxoPutsData(txoRows: string[][]) {
  ...
  async fetchLargeTradersTxoPosition(options?: { date: string }) {
    ...
    const callsData = this.parseTxoCallsData(txoRows);
    const putsData = this.parseTxoPutsData(txoRows);
