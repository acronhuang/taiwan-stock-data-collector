...
export class TpexScraperService {
  ...
  async fetchIndicesQuotes(options?: { date: string }) {
    ...
    // 驗證數值有效性
    if (closePrice === null || change === null || closePrice <= 0) return null;
    ...
    return [{
      ...
      change: change,
      changePercent: change !== 0 ? +numeral(change).divide(openPrice).multiply(100).format('0.00') : 0,
    }];
  }
  ...
  async fetchIndicesTrades(options?: { date: string }) {
    ...
    const data = json.tables[0].data.map(row => {
      const [sector, tradeValue, tradeWeight, volume, volumeWeight] = row;
      return {
        date,
  ...
  async fetchEquitiesInstInvestorsTrades(options?: { date: string }) {
    ...
    return json.tables[0].data.map(row => {
      ...
      const data: Record<string, any> = { date, symbol, name };
      data.finiNetBuySell = numeral(values[2]).value();    // 外資買賣超 [4]
      data.sitcNetBuySell = numeral(values[5]).value();    // 投信買賣超 [7] 
      data.dealersNetBuySell = numeral(values[18]).value(); // 自營+券商買賣超 [22]
      return data;
    });
  }
